...
- **Scheduled messages** with enqueue-time semantics, plus a dashboard view and admin API to list them and time-shift or deliver them early, per namespace (see [Scheduled message admin](#scheduled-message-admin))
...
shift everything in the view earlier or later, or deliver it all now. The same operations are a
JSON API on the dashboard port, which is handy for fast-forwarding time in tests. Every route is
scoped to one namespace, and `?entity=` narrows it to one queue or topic.

The API only covers what AMQP can't express. There are no routes for a single message:
cancelling or rescheduling one is `CancelScheduledMessageAsync` (then `ScheduleMessageAsync`)
from the client.
...
Shifted messages keep their sequence numbers, so `CancelScheduledMessageAsync` still works on
them. Anything shifted into the past is delivered on the next poll (within 500 ms).
